...
def visualize_sample(nusc: NuScenes,
    ...
    # legend_elements.append(Line2D([0], [0], color='black', lw=2, label='Ego Vehicle'))
    from matplotlib import cm
    from matplotlib.colors import Normalize
    # 5. SSR 和 baseline 预测轨迹
    def render_plan(plan_results, cmap_name='Blues', label='SAR traj'):
        ...
        plan_vecs = np.concatenate([
            np.stack((np.linspace(p[0, 0], p[1, 0], 51),
                    np.linspace(p[0, 1], p[1, 1], 51)), axis=-1)[:-1]
            for p in plan_traj
        ], axis=0)
        ...
        # 使用 colormap 映射颜色
        cmap = cm.get_cmap(cmap_name)
        norm = Normalize(vmin=0, vmax=plan_vecs.shape[0])
        colors = [cmap(norm(i)) for i in range(plan_vecs.shape[0])]

        lc = LineCollection(plan_vecs, colors=colors, linewidths=2.0, linestyles='solid', label=label)
    ...
    #     handler = render_plan(pred_data['plan_results'][sample_token], cmap_name='winter_r', label='SAR Trajectory')
    ...
    'Paired', 'Paired_r', 'Pastel1', 'Pastel1_r', 'Pastel2', 'Pastel2_r', 'PiYG', 
    ...
    if baseline_data is not None:
        handler = render_plan(baseline_data['plan_results'][sample_token], cmap_name='Greens_r', label='VAD Trajectory')
        legend_elements.append(handler)
    ...
    if len(ego_traj) >= 2:
        ...
        plan_vecs = np.stack((plan_vecs[:-1], plan_vecs[1:]), axis=1)
        # colors = color_map(np.linspace(0, 1, plan_vecs.shape[0]), cmap='autumn')
        # axes.add_collection(LineCollection(plan_vecs, colors=colors, linewidths=1, linestyles='solid'))
        lc_gt = LineCollection(plan_vecs, colors=['darkorange'], linewidths=1.5, linestyles='solid', label='GT traj')
...
def parse_args():
    ...
    parser.add_argument('--result-path', default="test/SSR_pts_motion_e2e_v3/Tue_Jul__1_02_51_56_2025/pts_bbox/results_nusc.pkl",help='inference result file path')
    parser.add_argument('--ssr-result-path', default="vad.pkl", help='inference result file path')
...
if __name__ == '__main__':
    ...
    for id in tqdm(range(6000)):#len(sample_token_list)
        if id % 6 == 0 :
            ...
            if out_path is not None:
                save_file = os.path.join(out_path, f"VAD_GT/{sample_token}_vis.png")

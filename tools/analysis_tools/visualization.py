...
def lidiar_render(sample_token, data, out_path=None, baseline_data=None, out_name=None, traj_use_perstep_offset=True):
    bbox_gt_list = []
    bbox_pred_list = []
    ...
    # bbox_anns = data['plan_results'][sample_token]
    ...
    visualize_sample(nusc, sample_token, gt_annotations, 
                     savepath=out_path, traj_use_perstep_offset=traj_use_perstep_offset, pred_data=data,baseline_data=baseline_data)
...
#     for box_est, box_est_global in zip(boxes_gt, boxes_gt_global):
...
#     plan_cmd = np.argmax(baseline_data['plan_results'][sample_token][1][0,0,0])
#     plan_traj = baseline_data['plan_results'][sample_token][0][plan_cmd]
#     plan_traj[abs(plan_traj) < 0.01] = 0.0
#     plan_traj = plan_traj.cumsum(axis=0)
#     plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
#     plan_traj = np.stack((plan_traj[:-1], plan_traj[1:]), axis=1)
...
#     for _ in range(6):
#         try:
#             sample = nusc.get('sample', future_token)
#             sd = nusc.get('sample_data', sample['data']['LIDAR_TOP'])
#             ego_pose = nusc.get('ego_pose', sd['ego_pose_token'])
#             pos = np.array(ego_pose['translation'])  # 世界坐标下的 ego xyz
#             ego_traj.append(pos)
#             if sample['next'] == "":
#                 break
#             future_token = sample['next']
#         except:
#             break
...
#         # 转换为 LIDAR 坐标系
#         pose = np.array(pose_record['translation'])
#         rot = Quaternion(pose_record['rotation'])
#         calib_trans = np.array(cs_record['translation'])
#         calib_rot = Quaternion(cs_record['rotation'])

#         traj_lidar = []
#         for pt in ego_traj:
#             # world -> ego -> lidar
#             pt_local = rot.inverse.rotate(pt - pose)
#             pt_lidar = calib_rot.inverse.rotate(pt_local - calib_trans)
#             traj_lidar.append(pt_lidar[:2])

#         traj_lidar = np.array(traj_lidar)

#         # ✅ 构造线段
#         if traj_lidar.shape[0] >= 2:
#             plan_traj = np.stack((traj_lidar[:-1], traj_lidar[1:]), axis=1)

#             # ✅ 平滑线段插值
#             plan_vecs = []
#             for plan_vec_i in plan_traj:
#                 x_linspace = np.linspace(plan_vec_i[0, 0], plan_vec_i[1, 0], 51)
#                 y_linspace = np.linspace(plan_vec_i[0, 1], plan_vec_i[1, 1], 51)
#                 xy = np.stack((x_linspace, y_linspace), axis=1)
#                 xy_segments = np.stack((xy[:-1], xy[1:]), axis=1)
#                 plan_vecs.append(xy_segments)

#             plan_vecs = np.concatenate(plan_vecs, axis=0)

#             # ✅ 渲染
#             cmap = 'autumn'
#             y = np.sin(np.linspace(1/2*np.pi, 3/2*np.pi, plan_vecs.shape[0] + 1))
#             colors = color_map(y[:-1], cmap)
#             line_segments = LineCollection(plan_vecs, colors=colors, linewidths=1, linestyles='solid', cmap=cmap)
#             axes.add_collection(line_segments)
#         # axes.plot(traj_lidar[:, 0], traj_lidar[:, 1], color='gold', linestyle='-', marker='o', label='GT Trajectory', linewidth=2)
#     from matplotlib.lines import Line2D
#     axes.axes.xaxis.set_ticks([])
#     axes.axes.yaxis.set_ticks([])
#     legend_elements = [
#     # Line2D([0], [0], color='mediumseagreen', lw=1, label='Planning Area'),
#     Line2D([0], [0], color='blue', lw=2, label='SAR Trajectory'),
#     Line2D([0], [0], color='mediumseagreen', lw=2, label='SSR Trajectory'),
#     Line2D([0], [0], color='darkorange', lw=2, label='GT Trajectory'),
#     ]

#     axes.legend(handles=legend_elements, loc='upper right', fontsize='small', frameon=True) 
#     axes.axis('off')
...
def visualize_sample(nusc: NuScenes,
    ...
    # 2. 初始化画布
    fig, axes = plt.subplots(1, 1, figsize=(4, 4))
    plt.xlim(xmin=pc_range[0], xmax=pc_range[3])
    plt.ylim(ymin=pc_range[1], ymax=pc_range[4])
    axes.set_aspect('equal')

    # 3. 渲染地图 LiDARInstanceLines
    ...
    # if isinstance(gt_vecs_pts_loc, LiDARInstanceLines):
    #     sampled_pts = gt_vecs_pts_loc.fixed_num_sampled_points
    #     for line_pts, cls in zip(sampled_pts, gt_vecs_label):
    #         x = line_pts[:, 0]
    #         y = line_pts[:, 1]
    #         if map_classes:
    #             color = colors_plt[cls % len(colors_plt)]
    #         else:
    #             color = 'gray'
    #         axes.plot(x, y, color=color, linewidth=0.5, alpha=0.9)
    if isinstance(gt_vecs_pts_loc, LiDARInstanceLines):
        try:
            vec_tensor = gt_vecs_pts_loc.fixed_num_sampled_points  # [N, fixed_num, 2]
        except Exception as e:
            print("Failed to get fixed_num_sampled_points from gt_vecs_pts_loc:", e)
            vec_tensor = None
    else:
        vec_tensor = None

    if vec_tensor is not None:
        vec_tensor = vec_tensor.cpu().numpy()  # [N, fixed_num, 2]
        num_vecs = vec_tensor.shape[0]
        color_map_rgb = ['gray', 'orange', 'green', 'blue', 'purple', 'red', 'cyan', 'yellow']
        for i in range(num_vecs):
            pts = vec_tensor[i]
            label = int(gt_vecs_label[i]) if gt_vecs_label is not None else 0
            color = color_map_rgb[label % len(color_map_rgb)]
            axes.plot(pts[:, 0], pts[:, 1], color=color, linewidth=1.0, alpha=0.8, linestyle='--')
    
    # 4. 渲染 GT box
    boxes_gt_global = gt_boxes[sample_token]
    boxes_gt = boxes_to_sensor(boxes_gt_global, pose_record, cs_record)
    for box, box_global in zip(boxes_gt, boxes_gt_global):
        box.score = box_global.detection_score
        if box.name not in ['barrier', 'bicycle', 'traffic_cone']:
            box.render(axes, view=np.eye(4), colors=('tomato', 'tomato', 'tomato'), linewidth=1)

    ego_box_length = 1.2  # meters
    ego_box_width = 3.0   # meters
    ego_box = np.array([
        [ ego_box_length/2,  ego_box_width/2],
        [ ego_box_length/2, -ego_box_width/2],
        [-ego_box_length/2, -ego_box_width/2],
        [-ego_box_length/2,  ego_box_width/2],
        [ ego_box_length/2,  ego_box_width/2],  # 闭合
    ])
    axes.plot(ego_box[:, 0], ego_box[:, 1], color='red', linewidth=1.5, linestyle='-', label='Ego Vehicle')

    # 5. SSR 和 baseline 预测轨迹
    def render_plan(plan_results, color, label):
        plan_cmd = np.argmax(plan_results[1][0, 0, 0])
        plan_traj = plan_results[0][plan_cmd]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        plan_traj = plan_traj.cumsum(axis=0)
        plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
        plan_traj = np.stack((plan_traj[:-1], plan_traj[1:]), axis=1)
        plan_vecs = np.concatenate([
            np.stack((np.linspace(p[0, 0], p[1, 0], 51), np.linspace(p[0, 1], p[1, 1], 51)), axis=-1)[:-1]
            for p in plan_traj
        ], axis=0)
        plan_vecs = np.stack((plan_vecs[:-1], plan_vecs[1:]), axis=1)
        # 获取 colormap 并生成颜色序列
        # try:
        #     cmap = plt.get_cmap(color)
        # except:
        #     cmap = plt.get_cmap('viridis')  # 默认 fallback
        # colors = color_map(np.linspace(0, 1, plan_vecs.shape[0]), cmap)
        # lc = LineCollection(plan_vecs, colors=colors, linewidths=2.5, linestyles='solid', label=label)
        # axes.add_collection(lc)
        # 使用单色而非渐变
        lc = LineCollection(plan_vecs, colors=[color], linewidths=2.5, linestyles='solid', label=label)
        axes.add_collection(lc)
        return lc  # 用于 legend
    ...
    if pred_data is not None:
        handler = render_plan(pred_data['plan_results'][sample_token], color='blue', label='SAR')
        legend_elements.append(handler)

    if baseline_data is not None:
        handler = render_plan(baseline_data['plan_results'][sample_token], color='green', label='VAD')
        legend_elements.append(handler)

    
    # 6. 自车 GT 轨迹（未来帧 ego_pose）
    future_token = sample_token
    ego_traj = []
    ...
    if len(ego_traj) >= 2:
        ...
        plan_vecs = np.stack((plan_vecs[:-1], plan_vecs[1:]), axis=1)
        # colors = color_map(np.linspace(0, 1, plan_vecs.shape[0]), cmap='Reds')
        # lc_gt = LineCollection(plan_vecs, colors=colors, linewidths=2.5, linestyles='solid', label='GT')
        # axes.add_collection(lc_gt)
        lc_gt = LineCollection(plan_vecs, colors=['red'], linewidths=2.5, linestyles='solid', label='GT')
        axes.add_collection(lc_gt)
        legend_elements.append(lc_gt)

    # 7. 图例和保存
    axes.axis('off')
    
    axes.legend(handles=legend_elements, loc='upper right', fontsize='small', frameon=True,   # 或 'medium'
                                                        prop={'family': 'DejaVu Sans', 'weight': 'bold'})
    ############################################################################################################
    # def render_plan(plan_results, color_scheme, label, linewidth=1.8):
    #     """渲染单色系渐变轨迹（蓝色/绿色/红色）"""
    #     plan_cmd = np.argmax(plan_results[1][0, 0, 0])
    #     plan_traj = plan_results[0][plan_cmd]
    #     plan_traj[abs(plan_traj) < 0.01] = 0.0
    ...
    #     # 构建线段 (N, 2, 2)
    #     segments = np.stack((plan_traj[:-1], plan_traj[1:]), axis=1)
        
    #     # 创建高精度点序列 (51点/线段)
    #     all_points = []
    #     for seg in segments:
    #         points = np.stack((
    #             np.linspace(seg[0, 0], seg[1, 0], 51),
    #             np.linspace(seg[0, 1], seg[1, 1], 51)
    #         ), axis=1)[:-1]
    ...
    #     # 重建高精度线段
    #     segments_high_res = np.stack((all_points[:-1], all_points[1:]), axis=1)
        
    #     # 生成单色系渐变色谱
    #     num_segments = len(segments_high_res)
    #     colors = np.zeros((num_segments, 4))
        
    #     # 根据color_scheme选择渐变类型
    #     if color_scheme == 'blue':
    #         # 蓝色渐变：深蓝 -> 亮蓝
    #         for i in range(num_segments):
    #             t = i / max(num_segments - 1, 1)
    #             colors[i] = [0.0, 0.3*t, 0.7 + 0.3*t, 1.0]
        
    #     elif color_scheme == 'green':
    #         # 绿色渐变：深绿 -> 亮绿
    #         for i in range(num_segments):
    #             t = i / max(num_segments - 1, 1)
    #             colors[i] = [0.0, 0.4 + 0.6*t, 0.1*t, 1.0]
        
    #     elif color_scheme == 'red':
    #         # 红色渐变：深红 -> 亮橙
    #         for i in range(num_segments):
    #             t = i / max(num_segments - 1, 1)
    #             colors[i] = [0.7 + 0.3*t, 0.2*t, 0.0, 1.0]
    ...
    # if pred_data is not None:
    #     handler = render_plan(
    #         pred_data['plan_results'][sample_token], 
    #         color_scheme='blue',
    #         label='SAR',
    #         linewidth=2.0
    #     )
    ...
    # if baseline_data is not None:
    #     handler = render_plan(
    #         baseline_data['plan_results'][sample_token], 
    #         color_scheme='green',
    #         label='VAD',
    #         linewidth=2.0
    #     )
    ...
    #     # 构建线段
    #     if len(traj_lidar) > 1:
    #         segments = np.stack((traj_lidar[:-1], traj_lidar[1:]), axis=1)
    ...
    #             for i in range(num_segments):
    #                 t = i / max(num_segments - 1, 1)
    #                 colors[i] = [0.7 + 0.3*t, 0.2*t, 0.0, 1.0]  # 深红 -> 亮橙
    ...
    #     if i == 0:  # SAR
    #         handle.set_color((0.0, 0.3, 1.0))  # 亮蓝色
    #         handle.set_linewidth(2.5)
    #     elif i == 1:  # VAD
    #         handle.set_color((0.0, 1.0, 0.0))  # 亮绿色
    #         handle.set_linewidth(2.5)
    #     else:  # GT
    #         handle.set_color((1.0, 0.2, 0.0))  # 亮橙色
    #         handle.set_linewidth(2.5)
    fig.set_tight_layout(True)
    if savepath is not None:
        plt.savefig(f'{savepath}/bev_pred.png', bbox_inches='tight', dpi=200)
        plt.close()
    else:
        plt.show()
...
def render_sample_data(
    ...
    """
    lidiar_render(sample_toekn, pred_data, out_path=out_path,baseline_data=baseline_data,
...
def parse_args():
    ...
    parser.add_argument('--result-path', default="test/SAR_pts_motion_e2e_v2/Thu_Sep_25_08_53_16_2025/pts_bbox/results_nusc.pkl",help='inference result file path')
    parser.add_argument('--ssr-result-path', default="", help='inference result file path') #vad.pkl
    parser.add_argument('--save-path', default="vis_results", help='the dir to save visualization results')
...
if __name__ == '__main__':
    ...
    baseline_result_path = args.ssr_result_path
    out_path = args.save_path
    bevformer_results = mmcv.load(inference_result_path)
    
    try:
        os.makedirs(baseline_result_path, exist_ok=True)
        ssr_bevformer_results = mmcv.load(baseline_result_path)
    except:
        ssr_bevformer_results = None
    sample_token_list = list(bevformer_results['plan_results'].keys())
        ...
        if id % 1 == 0 :
            ...
            for cam in cams:
                ...
                if sensor_modality in ['lidar', 'radar']:
                    ...
                    #                                                             pred_anns=boxes)
                    data_path, boxes_gt, camera_intrinsic = nusc.get_sample_data(sample_data_token, box_vis_level=BoxVisibility.ANY)
                    ...
                    if cam == 'CAM_FRONT':
                        ...
                        ####################################################
                        if ssr_bevformer_results is not None:
                            plan_cmd = np.argmax(ssr_bevformer_results['plan_results'][sample_token][1][0,0,0])
                            plan_traj = ssr_bevformer_results['plan_results'][sample_token][0][plan_cmd]
                            plan_traj[abs(plan_traj) < 0.01] = 0.0
                            plan_traj = plan_traj.cumsum(axis=0)
                        ...
                        plan_traj_gt = bevformer_results['plan_gts'][sample_token]
                        plan_traj_gt[abs(plan_traj_gt) < 0.01] = 0.0
                        plan_traj_gt = plan_traj_gt.cumsum(axis=0)

                        plan_traj_gt = np.concatenate((
                            plan_traj_gt[:, [0]],
                            plan_traj_gt[:, [1]],
                            -1.0*np.ones((plan_traj_gt.shape[0], 1)),
                            np.ones((plan_traj_gt.shape[0], 1)),
                        ), axis=1)
                        # add the start point in lcf
                        plan_traj_gt = np.concatenate((np.zeros((1, plan_traj_gt.shape[1])), plan_traj_gt), axis=0)
                        # plan_traj[0, :2] = 2*plan_traj[1, :2] - plan_traj[2, :2]
                        plan_traj_gt[0, 0] = 0.3
                        plan_traj_gt[0, 2] = -1.0
                        plan_traj_gt[0, 3] = 1.0
                        ...
                        viewpad[:camera_intrinsic.shape[0], :camera_intrinsic.shape[1]] = camera_intrinsic
                        lidar2img_rt = (viewpad @ lidar2cam_rt.T)
                        plan_traj_gt = lidar2img_rt @ plan_traj_gt.T
                        plan_traj_gt = plan_traj_gt[0:2, ...] / np.maximum(
                            plan_traj_gt[2:3, ...], np.ones_like(plan_traj_gt[2:3, ...]) * 1e-5)
                        plan_traj_gt = plan_traj_gt.T
                        plan_traj_gt = np.stack((plan_traj_gt[:-1], plan_traj_gt[1:]), axis=1)

                        plan_vecs = None
                        for i in range(plan_traj_gt.shape[0]):
                            plan_vec_i = plan_traj_gt[i]
                            x_linspace = np.linspace(plan_vec_i[0, 0], plan_vec_i[1, 0], 51)
                            y_linspace = np.linspace(plan_vec_i[0, 1], plan_vec_i[1, 1], 51)
                            xy = np.stack((x_linspace, y_linspace), axis=1)
                            xy = np.stack((xy[:-1], xy[1:]), axis=1)
            ...
            if out_path is not None:
                    save_file = os.path.join(out_path, f"SAR_VAD_GT_21/{sample_token}_vis.png")

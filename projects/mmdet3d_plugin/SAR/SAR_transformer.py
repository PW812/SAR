...
from projects.mmdet3d_plugin.SAR.modules.decoder import CustomMSDeformableAttention
from projects.mmdet3d_plugin.SAR.modules.temporal_self_attention import TemporalSelfAttention
from projects.mmdet3d_plugin.SAR.modules.spatial_cross_attention import MSDeformableAttention3D
...
class SARPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(SARPerceptionTransformer, self).__init__(**kwargs)
    ...
    def forward(self,
        ...
        # pts_bev = bev_queries.contiguous()
        if self.decoder is not None and pts_feat is not None:
            # [L, Q, B, D], [L, B, Q, D]
            inter_states, inter_references = self.decoder(
                query=query,
                key=None,
                value=pts_feat,#bev_embed
                query_pos=query_pos,
        ...
        elif self.decoder is not None and pts_feat is None:
            # [L, Q, B, D], [L, B, Q, D]
            inter_states, inter_references = self.decoder(
                query=query,
                key=None,
                value=bev_embed,#bev_embed
                query_pos=query_pos,
                reference_points=reference_points,
                reg_branches=reg_branches,
                cls_branches=cls_branches,
        ...
        return (
            bev_embed.permute(1, 0, 2), inter_states, init_reference_out, inter_references_out)

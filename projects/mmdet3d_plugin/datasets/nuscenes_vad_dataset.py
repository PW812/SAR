...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def union2one(self, queue):
        ...
        if 'points' in queue[0]:
            ...
            points_ = pad_sequence(points_list, batch_first=True, padding_value=0)
        ...
        ###############################################################
        if 'sparse_depth' in queue[0]:
            sparse_depth_list = [each['sparse_depth'] for each in queue]
            sparse_depth_list = torch.stack(sparse_depth_list)
            queue[-1]['sparse_depth'] = DC(sparse_depth_list, cpu_only=False, stack=True)
        #################################################################
        metas_map = {}
        prev_pos = None
        prev_angle = None
        ...
        if points_ is not None:
            queue['points'] = DC(points_, cpu_only=False, stack=True)
        return queue
